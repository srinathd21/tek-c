<?php
// pending-approvals.php (Manager) — show ALL quotation requests pending manager approval
...
$sql = "
  SELECT 
    ...
    (
      ...
      AND q.status IN ('Submitted', 'With QS', 'QS Negotiated', 'Finalized')
    ) AS quotation_count,
    (
      ...
      WHERE q.quotation_request_id = qr.id 
      AND q.status IN ('Submitted', 'With QS', 'QS Negotiated', 'Finalized')
    ) AS min_quotation_amount,
    (
      SELECT MAX(q.grand_total) 
      FROM quotations q 
      WHERE q.quotation_request_id = qr.id 
      AND q.status IN ('Submitted', 'With QS', 'QS Negotiated', 'Finalized')
    ) AS max_quotation_amount,
    (
    ...
    (
      ...
      FROM quotations q 
      WHERE q.quotation_request_id = qr.id 
      AND q.manager_decision = 'Pending'
    ) AS pending_decision_count
  FROM quotation_requests qr
  JOIN sites s ON qr.site_id = s.id
  LEFT JOIN clients c ON s.client_id = c.id
  WHERE qr.requested_by = ? 
    AND qr.status IN ('QS Finalized')
  ORDER BY 
    qr.priority = 'Urgent' DESC,
    qr.priority = 'High' DESC,
    qr.updated_at DESC
";
...
foreach ($requests as $req) {
    ...
    $total_quotation_count += intval($req['quotation_count'] ?? 0);
}
          ...
          <div class="col-12 col-md-6 col-xl-3">
            <div class="stat-card">
              ...
              <div>
                ...
                <div class="stat-value"><?php echo (int)$total_quotation_count; ?></div>
              </div>
            </div>
          </div>
        </div>
                ...
                ?>
                  <div class="approval-card <?php echo $cardClass; ?>">
                    ...
                    <div class="approval-kv">
                      ...
                      <div class="approval-row">
                        ...
                        <div class="approval-val">
                          <span class="amount-badge">
                            <i class="bi bi-file-text"></i> <?php echo intval($req['quotation_count'] ?? 0); ?> received
                          </span>
                        </div>
                      </div>
                      ...
                      <div class="approval-row">
                        <div class="approval-key">Finalized</div>
                        <div class="approval-val fw-800 text-success">
                          ₹ <?php echo number_format($req['finalized_amount'], 2); ?>
                        </div>
                      </div>
                      <?php endif; ?>

                      <?php if (!empty($req['min_quotation_amount']) && empty($req['finalized_amount'])): ?>
                      <div class="approval-row">
                        <div class="approval-key">Range</div>
                        <div class="approval-val">
                          ₹ <?php echo number_format($req['min_quotation_amount'], 2); ?> - 
                          ₹ <?php echo number_format($req['max_quotation_amount'], 2); ?>
                        </div>
                      </div>
                      <?php endif; ?>

                      <div class="approval-row">
                        <div class="approval-key">QS Finalized</div>
                        <div class="approval-val">
                          <?php echo getTimeAgo($req['finalized_at'] ?? $req['updated_at']); ?>
                        </div>
                      </div>
                    ...
                    <div class="approval-actions">
                      ...
                      <a href="approve-quotation.php?request_id=<?php echo $req['id']; ?>" class="btn-action approve" title="Approve">
                      ...
                      <div class="modal-content">
                        ...
                        <form action="process-quotation-decision.php" method="POST">
                          <div class="modal-body">
                            ...
                            <div class="form-text">
                              <i class="bi bi-info-circle"></i> This will notify the QS team and project engineers.
                            </div>
                          </div>
              ...
              <table id="pendingApprovalsTable" class="table align-middle mb-0 dt-responsive" style="width:100%">
                ...
                <?php foreach ($requests as $req): ?>
                  <tr class="<?php echo $req['priority'] === 'Urgent' ? 'table-danger' : ($req['priority'] === 'High' ? 'table-warning' : ''); ?>">
                    ...
                    <td>
                      <?php if (!empty($req['finalized_amount'])): ?>
                        <span class="fw-800 text-success">₹ <?php echo number_format($req['finalized_amount'], 2); ?></span>
                        <div class="proj-sub">Finalized</div>
                      <?php elseif (!empty($req['min_quotation_amount'])): ?>
                        <span class="fw-800">₹ <?php echo number_format($req['min_quotation_amount'], 2); ?></span>
                        <span class="proj-sub"> to </span>
                        <span class="fw-800">₹ <?php echo number_format($req['max_quotation_amount'], 2); ?></span>
                      <?php else: ?>
                        —
                      <?php endif; ?>
                    </td>
                    <td>
                      <span class="fw-800"><?php echo safeDate($req['finalized_at'] ?? $req['updated_at']); ?></span>
