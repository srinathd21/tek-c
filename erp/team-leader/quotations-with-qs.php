<?php
// quotations-with-qs.php (Team Lead) — show quotation requests submitted to QS team (status = 'With QS')
// TL can track progress of QS negotiation and see updates
...
// ---------- Fetch quotations with QS (status = 'With QS') ----------
$sql = "";
$params = [];
$types = "";
    ...
    $sql = "
        SELECT 
            ...
            (
                ...
                FROM quotations q 
                WHERE q.quotation_request_id = qr.id 
                AND q.status = 'QS Negotiated'
                ORDER BY q.qs_negotiated_at DESC 
                LIMIT 1
            ) AS negotiated_amount,
            (
...
foreach ($requests as $req) {
    ...
    if (!empty($req['negotiated_amount']) && !empty($req['original_amount'])) {
        $negotiated_count++;
        $savings_total += ($req['original_amount'] - $req['negotiated_amount']);
    } elseif (!empty($req['negotiated_amount'])) {
        $negotiated_count++;
    }
}
...
<head>
    ...
    <title>Quotations With QS - TEK-C</title>
        ...
        <div id="contentScroll" class="content-scroll">
            <div class="container-fluid maxw">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h1 class="h3 fw-bold text-dark mb-1">Quotations With QS</h1>
                        <p class="text-muted mb-0">Quotation requests under QS review and negotiation</p>
                    </div>
                    <div>
                ...
                <div class="row g-3 mb-3">
                    <div class="col-12 col-md-6 col-xl-3">
                        <div class="stat-card">
                            <div class="stat-ic blue"><i class="bi bi-arrow-right-square"></i></div>
                            <div>
                                <div class="stat-label">With QS</div>
                                <div class="stat-value"><?php echo (int)$total_with_qs; ?></div>
                            </div>
                        </div>
                    </div>
                    <div class="col-12 col-md-6 col-xl-3">
                ...
                <div class="row g-3 mb-3">
                    ...
                    <div class="col-md-4">
                        <div class="stat-card">
                            <div class="stat-ic purple"><i class="bi bi-check-circle"></i></div>
                            <div>
                                <div class="stat-label">Negotiated</div>
                                <div class="stat-value"><?php echo (int)$negotiated_count; ?></div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="stat-card">
                            <div class="stat-ic" style="background: #10b981;"><i class="bi bi-piggy-bank"></i></div>
                            <div>
                                <div class="stat-label">Total Savings</div>
                                <div class="stat-value"><?php echo formatCurrency($savings_total); ?></div>
                            </div>
                        </div>
                    </div>
                </div>

                <!-- Info Banner if no requests -->
                <?php if (empty($requests)): ?>
                <div class="alert alert-info alert-dismissible fade show mb-3" role="alert">
                    <i class="bi bi-info-circle-fill me-2"></i>
                    No quotation requests are currently with QS team. Submitted requests will appear here.
                    <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                </div>
                <?php endif; ?>
                ...
                <div class="panel mb-4">
                    <div class="panel-header">
                        <h3 class="panel-title">Under QS Review</h3>
                        ...
                        <div class="d-grid gap-3">
                            <?php if (empty($requests)): ?>
                                <div class="text-center py-4 text-muted">
                                    <i class="bi bi-arrow-right-square" style="font-size: 48px;"></i>
                                    <p class="mt-2 fw-bold">No requests with QS</p>
                                    <p class="small">Submitted requests will appear here for tracking.</p>
                                </div>
                            <?php else: ?>
                                <?php foreach ($requests as $req): 
                                    ...
                                    if (!empty($req['negotiated_amount'])) $cardClass .= ' negotiated';
                                ...
                                ?>
                                    <div class="request-card <?php echo $cardClass; ?>">
                                        ...
                                        <div class="progress-stats mb-2">
                                            ...
                                            <span class="stat-chip"><i class="bi bi-currency-rupee"></i> Lowest: <?php echo formatCurrency($req['lowest_amount']); ?></span>
                                        </div>

                                        <?php if (!empty($req['negotiated_amount'])): ?>
                                            <div class="mb-2">
                                                <span class="negotiation-badge">
                                                    <i class="bi bi-check-circle"></i> Negotiated: <?php echo formatCurrency($req['negotiated_amount']); ?>
                                                </span>
                                                <?php if (!empty($req['original_amount']) && $req['original_amount'] != $req['negotiated_amount']): ?>
                                                    <span class="savings-badge ms-1">
                                                        <i class="bi bi-piggy-bank"></i> Saved <?php echo formatCurrency($req['original_amount'] - $req['negotiated_amount']); ?>
                                                    </span>
                                                <?php endif; ?>
                                                <div class="proj-sub mt-1">with <?php echo e($req['negotiated_dealer'] ?? 'dealer'); ?></div>
                                            </div>
                                        <?php endif; ?>

                                        <?php if (!empty($req['qs_remarks'])): ?>
                                            <div class="qs-remarks">
                                                <i class="bi bi-chat-dots"></i> <strong>QS Notes:</strong><br>
                                                <?php echo e(strlen($req['qs_remarks']) > 100 ? substr($req['qs_remarks'], 0, 100) . '...' : $req['qs_remarks']); ?>
                                            </div>
                                        <?php endif; ?>
                                        ...
                                        <div class="request-kv mt-2">
                                            ...
                                            <div class="request-row">
                                                <div class="request-key">With QS Since</div>
                                                <div class="request-val">
                                                    <?php echo getTimeAgo($req['updated_at']); ?>
                                                </div>
                                            </div>
                    ...
                    <div class="d-none d-md-block">
                        <div class="table-responsive">
                            <table id="withQsTable" class="table align-middle mb-0 dt-responsive" style="width:100%">
                                <thead>
                                    <tr>
                                        <th>Request No.</th>
                                        <th>Title / Site</th>
                                ...
                                <?php foreach ($requests as $req): 
                                    ...
                                    $savings = (!empty($req['original_amount']) && !empty($req['negotiated_amount'])) ? ($req['original_amount'] - $req['negotiated_amount']) : 0;
                                ?>
                                    <tr class="<?php echo $req['priority'] === 'Urgent' ? 'table-danger' : ($req['priority'] === 'High' ? 'table-warning' : ''); ?>">
                                        ...
                                        <td class="text-center">
                                         ...
                                         <td>
                                            <span class="fw-700"><?php echo formatCurrency($req['lowest_amount']); ?></span>
                                        </td>
                                        <td>
                                            <?php if (!empty($req['negotiated_amount'])): ?>
                                                <span class="fw-700 text-success"><?php echo formatCurrency($req['negotiated_amount']); ?></span>
                                                <div class="proj-sub">with <?php echo e($req['negotiated_dealer'] ?? '—'); ?></div>
                                            <?php else: ?>
                                        ...
                                        <td>
                                            <?php if ($savings > 0): ?>
                                                <span class="badge bg-success"><?php echo formatCurrency($savings); ?></span>
                                            <?php else: ?>
                                                <span class="text-muted">—</span>
                                            <?php endif; ?>
                                        </td>
                                        <td>
                                            <div class="fw-700"><?php echo getTimeAgo($req['updated_at']); ?></div>
                                            <div class="proj-sub"><?php echo safeDate($req['updated_at']); ?></div>
                                        </td>
                                        <td>
                    ...
                    <div class="row g-3">
                        <div class="col-md-4">
                            <div class="d-flex gap-3">
                                ...
                                <div>
                                    <h6 class="fw-900 mb-1">Under Review</h6>
                                    <p class="small text-muted mb-0">QS team is reviewing and negotiating with dealers.</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="d-flex gap-3">
                                ...
                                <div>
                                    <h6 class="fw-900 mb-1">Cost Savings</h6>
                                    <p class="small text-muted mb-0">QS team negotiates better prices with dealers.</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="d-flex gap-3">
                                <div class="stat-ic orange" style="width: 40px; height: 40px; font-size: 16px;">
                                    <i class="bi bi-check-circle"></i>
                                </div>
                                <div>
                                    <h6 class="fw-900 mb-1">Finalized</h6>
                                    <p class="small text-muted mb-0">Once QS finalizes, you'll be notified for manager approval.</p>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    function initWithQsTable() {
        const isDesktop = window.matchMedia('(min-width: 768px)').matches;
        const tbl = document.getElementById('withQsTable');
        if (!tbl) return;

        if (isDesktop) {
            if (!$.fn.DataTable.isDataTable('#withQsTable')) {
                $('#withQsTable').DataTable({
                    responsive: true,
                    autoWidth: false,
                    scrollX: false,
                    pageLength: 10,
                    lengthMenu: [[10, 25, 50, 100, -1], [10, 25, 50, 100, 'All']],
                    order: [[7, 'desc']], // Sort by when sent to QS
                    columnDefs: [
                        { targets: [10], orderable: false, searchable: false } // Action column
                    ],
                    language: {
                        zeroRecords: "No quotations with QS",
    ...
    $(function () {
        initWithQsTable();
        window.addEventListener('resize', initWithQsTable);
    });
</script>
