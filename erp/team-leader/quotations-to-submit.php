<?php
// quotations-to-submit.php (Team Lead) — show quotation requests ready to submit to QS
// Status = 'Quotations Received' (after TL uploads quotations)
// Clicking Submit to QS changes status to 'With QS'
...
// ---------- Handle Submit to QS Action ----------
if (isset($_GET['action']) && $_GET['action'] === 'submit' && isset($_GET['id'])) {
    $request_id = intval($_GET['id']);
    
    // Update status to 'With QS' - send to QS team for negotiation
    ...
    if (mysqli_stmt_execute($stmt)) {
        $success = "Quotation request submitted to QS team successfully!";
        header("Location: quotations-to-submit.php?status=success&message=" . urlencode($success));
        exit();
    } else {
...
// ---------- Fetch quotations ready to submit (status = 'Quotations Received') ----------
$sql = "";
$params = [];
$types = "";
...
if (empty($tl_site_ids)) {
    ...
    $sql = "
        ...
        WHERE qr.status = 'Quotations Received'
...
<head>
    ...
    <title>Ready to Submit to QS - TEK-C</title>
        ...
        <div id="contentScroll" class="content-scroll">
            <div class="container-fluid maxw">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h1 class="h3 fw-bold text-dark mb-1">Ready to Submit to QS</h1>
                        <p class="text-muted mb-0">Quotation requests ready for QS team review and negotiation</p>
                    </div>
                    <div>
                ...
                <div class="row g-3 mb-3">
                    <div class="col-12 col-md-6 col-xl-3">
                        <div class="stat-card">
                            <div class="stat-ic blue"><i class="bi bi-send"></i></div>
                            <div>
                                <div class="stat-label">Ready to Submit</div>
                                <div class="stat-value"><?php echo (int)$total_ready; ?></div>
                            </div>
                        </div>
                    </div>
                    <div class="col-12 col-md-6 col-xl-3">
                ...
                <div class="row g-3 mb-3">
                    ...
                    <div class="col-md-6">
                        <div class="stat-card">
                            ...
                            <div>
                                <div class="stat-label">Avg Quotations/Request</div>
                                <div class="stat-value"><?php echo $total_ready > 0 ? number_format($total_quotations / $total_ready, 1) : '0'; ?></div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="alert alert-info alert-dismissible fade show mb-3" role="alert">
                    ...
                    No quotation requests ready to submit. Upload quotations from dealers in the <a href="quotations-in-progress.php" class="alert-link">In Progress</a> page.
                    <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                </div>
                <?php endif; ?>
                ...
                <div class="panel mb-4">
                    <div class="panel-header">
                        <h3 class="panel-title">Ready for QS Review</h3>
                        ...
                        <div class="d-grid gap-3">
                            <?php if (empty($requests)): ?>
                                <div class="text-center py-4 text-muted">
                                    ...
                                    <p class="mt-2 fw-bold">No requests ready to submit</p>
                                    <p class="small">Upload quotations to make them ready for submission.</p>
                                </div>
                            <?php else: ?>
                                ...
                                ?>
                                    <div class="request-card <?php echo $cardClass; ?>">
                                        ...
                                        <div class="progress-stats mb-2">
                                            ...
                                            <span class="stat-chip"><i class="bi bi-currency-rupee"></i> <?php echo formatCurrency($req['lowest_amount']); ?></span>
                                        </div>

                                        <div class="price-range mb-2">
                                            <span>Price Range:</span>
                                            <span class="price-low"><?php echo formatCurrency($req['lowest_amount']); ?></span>
                                            <span>-</span>
                                            <span class="price-high"><?php echo formatCurrency($req['highest_amount']); ?></span>
                                        </div>
                                        ...
                                        <div class="request-actions">
                                            ...
                                            <a href="?action=submit&id=<?php echo $req['id']; ?>" class="btn-action submit" onclick="return confirm('Submit this quotation request to QS team for review and negotiation?')" title="Submit to QS">
                    ...
                    <div class="d-none d-md-block">
                        <div class="table-responsive">
                            <table id="readyToSubmitTable" class="table align-middle mb-0 dt-responsive" style="width:100%">
                                <thead>
                                    <tr>
                                        <th>Request No.</th>
                                        <th>Title / Site</th>
                                ...
                                ?>
                                    <tr class="<?php echo $req['priority'] === 'Urgent' ? 'table-danger' : ($req['priority'] === 'High' ? 'table-warning' : ''); ?>">
                                        ...
                                        <td class="text-center">
                                         ...
                                         <td>
                                            <span class="fw-700 text-success"><?php echo formatCurrency($req['lowest_amount']); ?></span>
                                         </td>
                                         <td>
                                        ...
                                        <td class="text-end actions-col">
                                            ...
                                            <a href="?action=submit&id=<?php echo $req['id']; ?>" class="btn-action submit" onclick="return confirm('Submit to QS team?')" title="Submit to QS">
                    ...
                    <div class="row g-3">
                        <div class="col-md-4">
                            <div class="d-flex gap-3">
                                ...
                                <div>
                                    <h6 class="fw-900 mb-1">1. Collect Quotations</h6>
                                    <p class="small text-muted mb-0">Get quotations from multiple dealers.</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="d-flex gap-3">
                                ...
                                <div>
                                    <h6 class="fw-900 mb-1">2. Upload & Compare</h6>
                                    <p class="small text-muted mb-0">Upload all quotations and compare prices.</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="d-flex gap-3">
                                ...
                                <div>
                                    <h6 class="fw-900 mb-1">3. Submit to QS</h6>
                                    <p class="small text-muted mb-0">Submit for negotiation and finalization.</p>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    function initReadyToSubmitTable() {
        const isDesktop = window.matchMedia('(min-width: 768px)').matches;
        const tbl = document.getElementById('readyToSubmitTable');
        if (!tbl) return;

        if (isDesktop) {
            if (!$.fn.DataTable.isDataTable('#readyToSubmitTable')) {
                $('#readyToSubmitTable').DataTable({
                    responsive: true,
                    autoWidth: false,
                    scrollX: false,
                    pageLength: 10,
                    lengthMenu: [[10, 25, 50, 100, -1], [10, 25, 50, 100, 'All']],
                    order: [[5, 'asc']], // Sort by lowest amount
                    columnDefs: [
                        { targets: [9], orderable: false, searchable: false } // Action column
                    ],
                    language: {
                        zeroRecords: "No quotations ready to submit",
            ...
            if ($.fn.DataTable.isDataTable('#readyToSubmitTable')) {
                $('#readyToSubmitTable').DataTable().destroy();
            }
        }
    }

    $(function () {
        initReadyToSubmitTable();
        window.addEventListener('resize', initReadyToSubmitTable);
    });
</script>
