...
// Important: TL users may be assigned through sites.team_lead_employee_id, not only through site_project_engineers.
if ($hasTeamLeadCol) {
    $sites_query = "
        SELECT DISTINCT
            s.id,
        ...
        WHERE (
            spe_self.employee_id = ?
            OR s.team_lead_employee_id = ?
            OR s.manager_employee_id = ?
        )
          $deletedFilter
    ...
    mysqli_stmt_bind_param($sites_stmt, "iii", $current_employee_id, $current_employee_id, $current_employee_id);
} else {
    $sites_query = "
        SELECT DISTINCT
            s.id,
                        ...
                        <div class="panel">
                            ...
                            <form method="POST" enctype="multipart/form-data" id="regularizationForm">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <select name="selected_site_id" class="form-select" id="selected_site" required onchange="updateManagerInfo()">
                                            ...
                                            <?php foreach ($assigned_sites as $site): ?>
                                                ...
                                                <option value="<?php echo (int)$site['id']; ?>"
                                                        data-manager-id="<?php
                                                            if ($currentRoleKey === 'tl') {
                                                                echo e($site['manager_employee_id'] ?? '');
                                                            } elseif (in_array($currentRoleKey, ['manager', 'hr'], true)) {
                                                                echo '';
                                                            } else {
                                                                echo e(!empty($site['team_lead_employee_id']) ? $site['team_lead_employee_id'] : (!empty($site['fallback_team_lead_id']) ? $site['fallback_team_lead_id'] : $site['manager_employee_id']));
                                                            }
                                                        ?>"
                                        ...
                                        <?php if (empty($assigned_sites)): ?>
                                            <div class="form-hint text-danger">
                                                No assigned projects found. Check whether this employee is added as Project Engineer, Project TL, or Project Manager for any site.
                                            </div>
                                        <?php else: ?>
                                            <div class="form-hint">
                                                Approval route is hierarchy based: Project Engineer → TL, TL → Manager, Manager/HR → Admin. TL users will see the manager name beside each project.
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                ...
                                <div class="submit-row">
                                    ...
                                    <button type="submit" class="primary-btn" <?php echo empty($assigned_sites) ? 'disabled' : ''; ?>>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                <div class="panel text-center">
                                    ...
                                    <p class="small text-muted mb-0">Project Engineer → TL, TL → Manager, Manager/HR → Admin.</p>
                                </div>
                            </div>
                        </div>
                    </div>
