...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if ($action === 'accept') {
        // Accept the assignment and change status to 'Assigned'
        $update_query = "UPDATE quotation_requests SET status = 'Assigned', updated_at = NOW() WHERE id = ? AND project_engineer_id = ?";
        $stmt = mysqli_prepare($conn, $update_query);
        mysqli_stmt_bind_param($stmt, "ii", $request_id, $empId);
        if (mysqli_stmt_execute($stmt)) {
            $success = "Quotation request accepted successfully.";
        } else {
            $error = "Failed to accept quotation request.";
        }
        mysqli_stmt_close($stmt);
    } elseif ($action === 'reject') {
        // Reject the assignment (remove project_engineer_id)
        $update_query = "UPDATE quotation_requests SET project_engineer_id = NULL, project_engineer_name = NULL, status = 'Pending Assignment', updated_at = NOW() WHERE id = ? AND project_engineer_id = ?";
        $stmt = mysqli_prepare($conn, $update_query);
        mysqli_stmt_bind_param($stmt, "ii", $request_id, $empId);
        if (mysqli_stmt_execute($stmt)) {
            $success = "Quotation request rejected. It will be reassigned.";
        } else {
                  ...
                  <div class="request-card <?php echo $cardClass; ?>">
                    ...
                    <div class="request-actions">
                      ...
                      <?php if ($req['status'] === 'Pending Assignment'): ?>
                        <a href="?action=accept&id=<?php echo $req['id']; ?>" class="btn-action accept" onclick="return confirm('Accept this quotation request?')" title="Accept">
                          <i class="bi bi-check-lg"></i> Accept
                        </a>
                        <a href="?action=reject&id=<?php echo $req['id']; ?>" class="btn-action reject" onclick="return confirm('Reject this quotation request?')" title="Reject">
                          <i class="bi bi-x-lg"></i> Reject
                        </a>
                      <?php elseif ($req['status'] === 'Assigned'): ?>
                        <a href="manage-quotation.php?id=<?php echo $req['id']; ?>" class="btn-action start" title="Start Working">
              ...
              <table id="assignedQuotationsTable" class="table align-middle mb-0 dt-responsive" style="width:100%">
                ...
                ?>
                  <tr class="<?php echo $req['priority'] === 'Urgent' ? 'table-danger' : ($req['priority'] === 'High' ? 'table-warning' : ''); ?>">
                    ...
                    <td>
                      <?php if (!empty($req['estimated_budget']) && $req['estimated_budget'] > 0): ?>
                        <span class="fw-700">₹ <?php echo number_format($req['estimated_budget'], 2); ?></span>
                      <?php else: ?>
                        —
                      <?php endif; ?>
                    </td>
          ...
          <div class="row g-3">
            <div class="col-md-4">
              <div class="d-flex gap-3">
                ...
                <div>
                  <h6 class="fw-900 mb-1">New Requests</h6>
                  <p class="small text-muted mb-0">Accept or reject new assignments. Accept to start working.</p>
                </div>
              </div>
            </div>
            <div class="col-md-4">
              <div class="d-flex gap-3">
                ...
                <div>
                  <h6 class="fw-900 mb-1">In Progress</h6>
                  <p class="small text-muted mb-0">Contact dealers, collect quotations, and prepare submission.</p>
                </div>
              </div>
            </div>
            <div class="col-md-4">
              <div class="d-flex gap-3">
                ...
                <div>
                  <h6 class="fw-900 mb-1">Submit</h6>
                  <p class="small text-muted mb-0">Once quotations are ready, submit to manager for review.</p>
                </div>
              </div>
            </div>
          </div>
