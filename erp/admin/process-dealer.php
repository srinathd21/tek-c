...
function addActivityLog($conn, $employee_id, $employee_name, $employee_role, $action_type, $module_id, $module_name, $description) {
    if (!$conn) {
        return false;
    }

    $idColumn = null;
    if (columnExists($conn, 'activity_logs', 'employee_id')) {
        $idColumn = 'employee_id';
    } elseif (columnExists($conn, 'activity_logs', 'user_id')) {
        $idColumn = 'user_id';
    }
    ...
    if (columnExists($conn, 'activity_logs', 'employee_name')) {
        $columns[] = 'employee_name';
        $values[] = $employee_name;
        $types .= 's';
    } elseif (columnExists($conn, 'activity_logs', 'user_name')) {
        $columns[] = 'user_name';
        $values[] = $employee_name;
        $types .= 's';
    }

    if (columnExists($conn, 'activity_logs', 'role')) {
        $columns[] = 'role';
        $values[] = $employee_role;
        $types .= 's';
    } elseif (columnExists($conn, 'activity_logs', 'user_role')) {
        $columns[] = 'user_role';
        $values[] = $employee_role;
        $types .= 's';
    }

    if (columnExists($conn, 'activity_logs', 'action_type')) {
        $columns[] = 'action_type';
        $values[] = $action_type;
        $types .= 's';
    } elseif (columnExists($conn, 'activity_logs', 'action')) {
        $columns[] = 'action';
        $values[] = $action_type;
        $types .= 's';
    }

    if (columnExists($conn, 'activity_logs', 'module')) {
        $columns[] = 'module';
        $values[] = 'quotation_dealers';
        $types .= 's';
    }

    if (columnExists($conn, 'activity_logs', 'module_id')) {
        $columns[] = 'module_id';
        $values[] = (int)$module_id;
        $types .= 'i';
    }

    if (columnExists($conn, 'activity_logs', 'module_name')) {
        $columns[] = 'module_name';
        $values[] = $module_name;
        $types .= 's';
    }

    if (columnExists($conn, 'activity_logs', 'description')) {
        $columns[] = 'description';
        $values[] = $description;
        $types .= 's';
    }

    if (columnExists($conn, 'activity_logs', 'created_at')) {
        $columns[] = 'created_at';
    }
    ...
    $sql = "INSERT INTO activity_logs (" . implode(", ", $insertColumns) . ") VALUES (" . implode(", ", $placeholders) . ")";
...
try {
    ...
    // Some databases use employee_id instead of user_id, so detect columns before insert.
    addActivityLog(
        $conn,
        $user_id,
        $user_name,
