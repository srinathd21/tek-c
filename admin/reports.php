<?php
/**
 * reports.php (UPDATED + COMPLETE)
 * ✅ Added MOBILE cards view for BOTH:
 *    1) Projects list view (no site_id)
 *    2) Reports list view (with site_id + type=dpr/dar)
 * ✅ Kept Desktop DataTable
 * ✅ PRG already done for manager editable toggle
 * ✅ Safer helpers + fixed minor issues
 */
...
/** Money formatter (your missing function) */
    ...
    if ($dprIdToUpdate <= 0 || !in_array($newStatus, [1,2], true)) {
      ...
      if (!$stUpd) {
        ...
        if (mysqli_stmt_execute($stUpd) && mysqli_stmt_affected_rows($stUpd) >= 0) {
          $success = ($newStatus === 1) ? "DPR edit enabled successfully." : "DPR edit disabled successfully.";
        } else {
...
if (!$showProjects) {
  ...
  if (!$siteInfo) {
    ...
    if ($type === 'dpr') {
      $sql = "
        SELECT
          d.id, d.site_id, d.employee_id, d.dpr_no, d.dpr_date,
          d.weather, d.site_condition,
          d.manpower_json, d.machinery_json, d.material_json, d.work_progress_json, d.constraints_json,
          d.report_distribute_to, d.prepared_by, d.created_at, d.editable_status,
          s.project_name, s.project_location, s.project_type,
          c.client_name, c.company_name, c.mobile_number AS client_mobile, c.email AS client_email, c.state AS client_state, c.client_type,
      ...
      $sql = "
        SELECT
          r.id, r.site_id, r.employee_id, r.dar_no, r.dar_date,
          r.division, r.incharge, r.activities_json,
          r.report_distribute_to, r.prepared_by, r.created_at,
          s.project_name, s.project_location, s.project_type,
          c.client_name, c.company_name, c.mobile_number AS client_mobile, c.email AS client_email, c.state AS client_state, c.client_type,
          emp.full_name AS employee_name, emp.designation AS employee_designation
        FROM dar_reports r
...
<head>
  ...
  <style>
    ...
    .btn-export:hover { background: #0da271; color: white; }
    ...
    .status-inactive{ background: rgba(245, 158, 11, 0.1); color: #f59e0b; border: 1px solid rgba(245, 158, 11, 0.2); }
    .status-neutral { background: rgba(59, 130, 246, 0.10); color: #2563eb; border: 1px solid rgba(59, 130, 246, 0.2); }

    .title{ font-weight:800; font-size:13px; color:#1f2937; margin-bottom:2px; line-height:1.2; }
    .sub{ font-size:11px; color:#6b7280; font-weight:700; line-height:1.2; }
    ...
    .card-title{ font-weight:1000; font-size:14px; color:#111827; line-height:1.25; }
    .card-sub{ font-size:12px; color:#6b7280; font-weight:800; margin-top:2px; display:flex; flex-wrap:wrap; gap:8px; align-items:center; }
    .kv{ margin-top:10px; display:grid; gap:8px; }
    .rowkv{ display:flex; gap:10px; }
    .key{ flex:0 0 92px; color:#6b7280; font-weight:1000; font-size:12px; }
    .val{ flex:1 1 auto; font-weight:900; color:#111827; font-size:13px; line-height:1.25; word-break:break-word; }
    .actions{ margin-top:10px; display:flex; gap:8px; flex-wrap:wrap; }
    .actions a, .actions button, .actions select{ flex:1 1 auto; border-radius:12px; justify-content:center; font-weight:900; }
  ...
  <main class="main" aria-label="Main">
    ...
    <div id="contentScroll" class="content-scroll">
      <div class="container-fluid maxw">
        ...
        <?php if ($success): ?>
          <div class="alert alert-success alert-dismissible fade show" role="alert" style="box-shadow:var(--shadow);border:none;border-radius:var(--radius);">
        ...
        <?php if ($error): ?>
          <div class="alert alert-danger alert-dismissible fade show" role="alert" style="box-shadow:var(--shadow);border:none;border-radius:var(--radius);">
        ...
        <div class="row g-3 mb-3">
          <?php if ($showProjects): ?>
            <div class="col-12 col-md-6 col-xl-3"><div class="stat-card"><div class="stat-ic blue"><i class="bi bi-collection-fill"></i></div><div><div class="stat-label">Total Projects</div><div class="stat-value"><?php echo (int)$total; ?></div></div></div></div>
            <div class="col-12 col-md-6 col-xl-3"><div class="stat-card"><div class="stat-ic green"><i class="bi bi-lightning-fill"></i></div><div><div class="stat-label">Ongoing</div><div class="stat-value"><?php echo (int)$ongoing; ?></div></div></div></div>
            <div class="col-12 col-md-6 col-xl-3"><div class="stat-card"><div class="stat-ic yellow"><i class="bi bi-clock-fill"></i></div><div><div class="stat-label">Upcoming</div><div class="stat-value"><?php echo (int)$upcoming; ?></div></div></div></div>
            <div class="col-12 col-md-6 col-xl-3"><div class="stat-card"><div class="stat-ic red"><i class="bi bi-check2-circle"></i></div><div><div class="stat-label">Completed</div><div class="stat-value"><?php echo (int)$completed; ?></div></div></div></div>
          <?php else: ?>
            <div class="col-12 col-md-6 col-xl-3"><div class="stat-card"><div class="stat-ic blue"><i class="bi bi-collection-fill"></i></div><div><div class="stat-label">Total <?php echo strtoupper($type); ?></div><div class="stat-value"><?php echo (int)$total; ?></div></div></div></div>
            <div class="col-12 col-md-6 col-xl-3"><div class="stat-card"><div class="stat-ic green"><i class="bi bi-calendar-check-fill"></i></div><div><div class="stat-label">Today (<?php echo e($today); ?>)</div><div class="stat-value"><?php echo (int)$cntToday; ?></div></div></div></div>
            <div class="col-12 col-md-6 col-xl-3"><div class="stat-card"><div class="stat-ic yellow"><i class="bi bi-calendar2-month-fill"></i></div><div><div class="stat-label">This Month</div><div class="stat-value"><?php echo (int)$cntMonth; ?></div></div></div></div>
            <div class="col-12 col-md-6 col-xl-3"><div class="stat-card"><div class="stat-ic red"><i class="bi bi-calendar3"></i></div><div><div class="stat-label">This Year</div><div class="stat-value"><?php echo (int)$cntYear; ?></div></div></div></div>
          <?php endif; ?>
        </div>
        ...
        <div class="d-block d-md-none mb-4">
          <?php if ($showProjects): ?>
            <?php if (empty($projects)): ?>
              <div class="panel text-muted" style="font-weight:900;">No projects found.</div>
            <?php else: ?>
              <div class="d-grid gap-3">
                <?php foreach ($projects as $p): ?>
                  ...
                  <div class="proj-card">
                    <div class="card-top">
                      <div class="card-main">
                        <div class="card-title"><?php echo e($p['project_name'] ?? ''); ?></div>
                        <div class="card-sub">
                          <span><i class="bi bi-pin-map"></i> <?php echo e($p['project_location'] ?? ''); ?></span>
                          <span>•</span>
                          <span><i class="bi bi-kanban"></i> <?php echo e($p['project_type'] ?? ''); ?></span>
                        </div>
                        <div class="card-sub mt-1">
                          <span><i class="bi bi-person"></i> <?php echo e($clientLine); ?></span>
                        </div>
                      </div>
                      <span class="chip"><i class="bi bi-hash"></i> <?php echo (int)$p['id']; ?></span>
                    </div>

                    <div class="kv">
                      <div class="rowkv">
                        <div class="key">Value</div>
                        <div class="val">₹ <?php echo e(showMoney($p['contract_value'] ?? '')); ?> <span style="color:#6b7280;font-weight:900;">(PMC: ₹ <?php echo e(showMoney($p['pmc_charges'] ?? '')); ?>)</span></div>
                      </div>
                      <div class="rowkv">
                        <div class="key">Start</div>
                        <div class="val"><?php echo safeDate($p['start_date'] ?? ''); ?></div>
                      </div>
                      <div class="rowkv">
                        <div class="key">End</div>
                        <div class="val"><?php echo safeDate($p['expected_completion_date'] ?? ''); ?></div>
                      </div>
                    </div>

                    <div class="actions">
                      <a href="reports.php?site_id=<?php echo (int)$p['id']; ?>&type=dpr" class="btn btn-outline-primary btn-sm">
                        <i class="bi bi-folder2-open"></i> Reports
                      </a>
                      <a href="view-site.php?id=<?php echo (int)$p['id']; ?>" class="btn btn-outline-secondary btn-sm">
                        <i class="bi bi-building"></i> View
                      </a>
                    </div>
                  </div>
                <?php endforeach; ?>
          ...
          <?php else: ?>
            <?php if (empty($rows)): ?>
              <div class="panel text-muted" style="font-weight:900;">No reports found.</div>
            <?php else: ?>
              <div class="d-grid gap-3">
                <?php foreach ($rows as $r): ?>
                  <?php if ($type === 'dpr'): ?>
                    ...
                    <div class="rep-card">
                      <div class="card-top">
                        <div class="card-main">
                          <div class="card-title"><?php echo e($r['dpr_no'] ?? ''); ?></div>
                          <div class="card-sub">
                            <?php echo $badge; ?>
                            <span><i class="bi bi-calendar3"></i> <?php echo safeDate($dprDate); ?></span>
                          </div>
                        </div>
                        <a href="view-dpr.php?id=<?php echo (int)$r['id']; ?>" class="btn btn-outline-primary btn-sm">
                          <i class="bi bi-eye"></i> View
                        </a>
                      </div>

                      <div class="kv">
                        <div class="rowkv"><div class="key">Weather</div><div class="val"><?php echo e($weather !== '' ? $weather : '—'); ?></div></div>
                        <div class="rowkv"><div class="key">Site</div><div class="val"><?php echo e($siteCond !== '' ? $siteCond : '—'); ?></div></div>
                        <div class="rowkv"><div class="key">Counts</div><div class="val">MP: <?php echo (int)$mp; ?> • MAC: <?php echo (int)$mac; ?> • MAT: <?php echo (int)$mat; ?> • WRK: <?php echo (int)$wrk; ?><?php echo ($con>0 ? " • CON: ".(int)$con : ""); ?></div></div>
                        <div class="rowkv"><div class="key">Prepared</div><div class="val"><?php echo e($r['prepared_by'] ?? ($empName !== '' ? $empName : '—')); ?><?php echo ($empDesg !== '' ? " • ".e($empDesg) : ""); ?></div></div>
                      </div>

                      <div class="actions">
                        <?php if ($canEditThisDpr): ?>
                          <a href="edit-dpr.php?id=<?php echo (int)$r['id']; ?>" class="btn btn-outline-secondary btn-sm">
                        ...
                        <?php if ($canManageEditable): ?>
                          <form method="POST">
                            ...
                            <select name="editable_status" class="form-select form-select-sm" onchange="this.form.submit()">
                  ...
                  <?php else: ?>
                    ...
                    <div class="rep-card">
                      <div class="card-top">
                        <div class="card-main">
                          <div class="card-title"><?php echo e($r['dar_no'] ?? ''); ?></div>
                          <div class="card-sub">
                            <?php echo $badge; ?>
                            <span><i class="bi bi-calendar3"></i> <?php echo safeDate($darDate); ?></span>
                          </div>
                        </div>
                        <a href="view-dar.php?id=<?php echo (int)$r['id']; ?>" class="btn btn-outline-primary btn-sm">
                          <i class="bi bi-eye"></i> View
                        </a>
                      </div>

                      <div class="kv">
                        <div class="rowkv"><div class="key">Division</div><div class="val"><?php echo e($division !== '' ? $division : '—'); ?></div></div>
                        <div class="rowkv"><div class="key">Incharge</div><div class="val"><?php echo e($incharge !== '' ? $incharge : '—'); ?></div></div>
                        <div class="rowkv"><div class="key">Activities</div><div class="val"><?php echo (int)$actCount; ?></div></div>
                        <div class="rowkv"><div class="key">Prepared</div><div class="val"><?php echo e($r['prepared_by'] ?? ($empName !== '' ? $empName : '—')); ?><?php echo ($empDesg !== '' ? " • ".e($empDesg) : ""); ?></div></div>
                      </div>
                    </div>
                  <?php endif; ?>
        ...
        <div class="panel mb-4 d-none d-md-block">
          <div class="panel-header">
            <h3 class="panel-title"><?php echo $showProjects ? 'Project Directory' : strtoupper($type).' Directory'; ?></h3>
                  ...
                  <?php foreach ($projects as $p): ?>
                    ...
                    <tr>
                      <td>
                        ...
                        <div class="sub"><i class="bi bi-hash"></i> ID: <?php echo (int)$p['id']; ?></div>
                      </td>
                      <td>
                        <div class="title"><?php echo e($clientLine); ?></div>
                        <div class="sub">
                          <?php if (!empty($p['client_state'])): ?><span class="me-2"><i class="bi bi-geo-alt"></i> <?php echo e($p['client_state']); ?></span><?php endif; ?>
                          <?php if (!empty($p['client_email'])): ?><span class="me-2"><i class="bi bi-envelope"></i> <?php echo e($p['client_email']); ?></span><?php endif; ?>
                        </div>
                      </td>
                      <td>
                        <div class="title"><?php echo e($p['project_type'] ?? ''); ?></div>
                        <div class="sub"><i class="bi bi-pin-map"></i> <?php echo e($p['project_location'] ?? ''); ?></div>
                      </td>
                      <td>
                    ...
                    <?php if ($type === 'dpr'): ?>
                      ...
                      <tr>
                        <td>
                          ...
                          <div class="sub"><?php echo $badge; ?> <span class="ms-2"><i class="bi bi-calendar3"></i> <?php echo safeDate($dprDate); ?></span></div>
                        </td>
                        <td>
                        ...
                        <td>
                          ...
                          <?php if ($con > 0): ?><div class="mt-1 chip"><i class="bi bi-exclamation-circle"></i> Constraints: <?php echo (int)$con; ?></div><?php endif; ?>
                        </td>
                        <td>
                          <div class="title"><?php echo e($r['prepared_by'] ?? ($empName !== '' ? $empName : '—')); ?></div>
                          <div class="sub">
                            <?php if ($empName !== ''): ?>
                              <i class="bi bi-person-badge"></i> <?php echo e($empName); ?><?php if ($empDesg !== ''): ?> • <?php echo e($empDesg); ?><?php endif; ?>
                            <?php else: ?><i class="bi bi-person"></i> —<?php endif; ?>
                          </div>
                        </td>
                        <td class="text-end actions-col">
                          <a href="view-dpr.php?id=<?php echo (int)$r['id']; ?>" class="btn-action" title="View DPR"><i class="bi bi-eye"></i></a>
                          <?php if ($canEditThisDpr): ?>
                            <a href="edit-dpr.php?id=<?php echo (int)$r['id']; ?>" class="btn-action" title="Edit DPR"><i class="bi bi-pencil-square"></i></a>
                          <?php endif; ?>
                          <?php if ($canManageEditable): ?>
                            <form method="POST" class="d-inline-flex align-items-center gap-1 ms-1">
                              ...
                              <input type="hidden" name="return_site_id" value="<?php echo (int)$siteId; ?>">
                              <input type="hidden" name="return_type" value="dpr">
                              <select name="editable_status" class="form-select form-select-sm" style="width:auto; min-width:120px;" onchange="this.form.submit()">
                                <option value="1" <?php echo ($editableStatus === 1 ? 'selected' : ''); ?>>Rejected</option>
                                <option value="2" <?php echo ($editableStatus === 2 ? 'selected' : ''); ?>>Approved</option>
                              </select>
                            </form>
                    ...
                    <?php else: ?>
                      ...
                      <tr>
                        <td>
                          ...
                          <div class="sub"><?php echo $badge; ?> <span class="ms-2"><i class="bi bi-calendar3"></i> <?php echo safeDate($darDate); ?></span></div>
                        </td>
                        <td>
                        ...
                        <td>
                          ...
                          <?php if (!empty($r['report_distribute_to'])): ?><div class="mt-1 sub"><i class="bi bi-send"></i> <?php echo e($r['report_distribute_to']); ?></div><?php endif; ?>
                        </td>
                        <td>
                          <div class="title"><?php echo e($r['prepared_by'] ?? ($empName !== '' ? $empName : '—')); ?></div>
                          <div class="sub"><?php if ($empName !== ''): ?><i class="bi bi-person-badge"></i> <?php echo e($empName); ?><?php if ($empDesg !== ''): ?> • <?php echo e($empDesg); ?><?php endif; ?><?php else: ?><i class="bi bi-person"></i> —<?php endif; ?></div>
                        </td>
                        <td class="text-end actions-col">
                          <a href="view-dar.php?id=<?php echo (int)$r['id']; ?>" class="btn-action" title="View DAR"><i class="bi bi-eye"></i></a>
                        </td>
                      </tr>
                    <?php endif; ?>
                  <?php endforeach; ?>
